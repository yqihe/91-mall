// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package model

import (
	"time"
)

const TableNameUmsMember = "ums_member"

// UmsMember 会员表
type UmsMember struct {
	ID                    int64     `gorm:"column:id;primaryKey;autoIncrement:true" json:"id"`
	MemberLevelID         int64     `gorm:"column:member_level_id" json:"member_level_id"`
	Username              string    `gorm:"column:username;comment:用户名" json:"username"`                              // 用户名
	Password              string    `gorm:"column:password;comment:密码" json:"password"`                               // 密码
	Nickname              string    `gorm:"column:nickname;comment:昵称" json:"nickname"`                               // 昵称
	Phone                 string    `gorm:"column:phone;comment:手机号码" json:"phone"`                                   // 手机号码
	Status                int32     `gorm:"column:status;comment:帐号启用状态:0->禁用；1->启用" json:"status"`                   // 帐号启用状态:0->禁用；1->启用
	CreateTime            time.Time `gorm:"column:create_time;comment:注册时间" json:"create_time"`                       // 注册时间
	Icon                  string    `gorm:"column:icon;comment:头像" json:"icon"`                                       // 头像
	Gender                int32     `gorm:"column:gender;comment:性别：0->未知；1->男；2->女" json:"gender"`                   // 性别：0->未知；1->男；2->女
	Birthday              time.Time `gorm:"column:birthday;comment:生日" json:"birthday"`                               // 生日
	City                  string    `gorm:"column:city;comment:所做城市" json:"city"`                                     // 所做城市
	Job                   string    `gorm:"column:job;comment:职业" json:"job"`                                         // 职业
	PersonalizedSignature string    `gorm:"column:personalized_signature;comment:个性签名" json:"personalized_signature"` // 个性签名
	SourceType            int32     `gorm:"column:source_type;comment:用户来源" json:"source_type"`                       // 用户来源
	Integration           int32     `gorm:"column:integration;comment:积分" json:"integration"`                         // 积分
	Growth                int32     `gorm:"column:growth;comment:成长值" json:"growth"`                                  // 成长值
	LuckeyCount           int32     `gorm:"column:luckey_count;comment:剩余抽奖次数" json:"luckey_count"`                   // 剩余抽奖次数
	HistoryIntegration    int32     `gorm:"column:history_integration;comment:历史积分数量" json:"history_integration"`     // 历史积分数量
}

// TableName UmsMember's table name
func (*UmsMember) TableName() string {
	return TableNameUmsMember
}
